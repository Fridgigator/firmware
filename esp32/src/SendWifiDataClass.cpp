...
SendWifiDataClass::SendWifiDataClass(vector<WiFiStorage> &wifi) {
    ...
    *p = Packet_init_zero;
    ...
    pb_size_t sizeOfPacketVec = sizeof p->type.wifi.data / sizeof p->type.wifi.data[0];
    ...
    p->type.wifi = WiFiVector{.data_count = static_cast<pb_size_t>(std::min((unsigned int) sizeOfPacketVec,
                                                                            (unsigned int) wifi.size()))};
    for (int i = 0; i < sizeOfPacketVec && i < payload.size(); i++) {
        p->type.wifi.data[i] = payload.at(i);
    }
    int status = pb_encode(&output, Packet_fields, p.get());
    if (!status) {
