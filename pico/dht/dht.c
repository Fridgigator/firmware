...
int dht_init(dht_t *dht, dht_model_t model, PIO pio, uint8_t data_pin, bool pull_up) {
...
int dht_start_measurement(dht_t *dht) {
    if(dht->pio == NULL) {
        return 1;
    }
    if (pio_sm_is_enabled(dht->pio, dht->sm)){
        return 1;
    }
